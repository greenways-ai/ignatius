(ns gwdb.ledger.workspace-proposal
...
(defn.pg ^{:- [:text]}
  proposal-name
  "Derives one immutable proposal ref name from the exact candidate root."
  {:added "0.13"}
  [:bytea i-desired-root]
  (return
   (|| "proposal/" (pg/encode i-desired-root "hex"))))

(defn.pg ^{:- [:bytea]}
  workspace-proposal-intent-value
  "Constructs one canonical create-only proposal publication intent."
  {:added "0.13"}
  [:bytea i-workspace-id-root
   :bytea i-address-root
   :bytea i-desired-root]
  (let [(:text v-scope) (-/proposal-scope i-workspace-id-root)
        (:text v-name) (-/proposal-name i-desired-root)
        (:bytea v-empty-map)
        ...
        (:bytea v-expected)
        (workspace/record-assoc
         v-name-record "ref/expected-root" (value/put-nil))
        (:bytea v-desired)
        (workspace/record-assoc
        ...
        (workspace/record-assoc
         ...
         (value/put-keyword "proposal-publication-v1"))]
    (return
     (workspace/record-assoc
      v-policy "ref/metadata" v-empty-map))))

(defn.pg ^{:- [:boolean]}
  workspace-proposal-intent-valid
  "Reconstructs the constrained proposal intent and compares its exact root."
  {:added "0.13"}
  [:bytea i-intent-root
   :bytea i-workspace-id-root
   :bytea i-address-root
  ...
  (return
   (== i-intent-root
       (-/workspace-proposal-intent-value
        i-workspace-id-root i-address-root i-desired-root))))

(defn.pg ^{:- [:text]}
  workspace-proposal-transition-error
  "Returns the first proposal publication failure, or SQL null."
  {:added "0.13"}
  [:bytea i-workspace-id-root
   :bytea i-desired-root]
    ...
    (cond (or [o-workspace-id :is-null]
          ...
          (not
           (scoped-ref/ref-part-valid
            (-/proposal-scope i-workspace-id-root)))
...
(defn.pg ^{:- [:jsonb]}
  workspace-proposal-signing-request
  "Returns standard transaction bytes for one create-only proposal."
  {:added "0.13"}
  [:text i-network
   :bytea i-public-key
   :bytea i-workspace-id-root
  ...
  (let [o-head (block/head-get i-network)
        ...
        (-/workspace-proposal-transition-error
         i-workspace-id-root i-desired-root)
        _ (pg/assert [v-transition-error :is-null]
                     [:ledger/invalid-workspace-proposal
                      v-transition-error])
        (:bytea v-intent-root)
        (-/workspace-proposal-intent-value
         i-workspace-id-root v-address-root i-desired-root)
        _ (pg/assert
           (-/workspace-proposal-intent-valid
            v-intent-root i-workspace-id-root
            v-address-root i-desired-root)
           [:ledger/invalid-workspace-proposal-intent])
    ...
    (return
     (pg/jsonb-build-object
      ...
      "scope" (-/proposal-scope i-workspace-id-root)
      "name" (-/proposal-name i-desired-root)
      "expected_root" nil
...
(defn.pg ^{:- [:jsonb]}
  workspace-proposal-submit
  "Atomically publishes one signed immutable proposal into the linear chain.

  Duplicate publication conflicts before transaction insertion, sequence
  advancement or global block commitment."
  {:added "0.13"}
  [:text i-network
   :bytea i-public-key
   :bigint i-sequence
  ...
  (let [o-head (block/head-lock i-network)
        ...
        _ (pg/assert [v-transition-error :is-null]
                     [:ledger/invalid-workspace-proposal
                      v-transition-error])
        (:text v-scope) (-/proposal-scope i-workspace-id-root)
        (:text v-name) (-/proposal-name i-desired-root)
        (:bytea v-intent-root)
        (-/workspace-proposal-intent-value
         i-workspace-id-root v-address-root i-desired-root)
        ...
        _ (pg/assert
           ...
           [:ledger/invalid-workspace-proposal-signature])
        o-cas
        (scoped-ref/scoped-ref-compare-and-set
         v-scope v-name nil i-desired-root v-address-root)
          ...
          _ (pg/assert
             (and (== (:text (:->> o-receipt "status")) "ok")
                  (== (:bytea (:->> o-receipt "result_root"))
                      v-intent-root))
             [:ledger/workspace-proposal-receipt-mismatch])
          (:bytea v-state-root)
